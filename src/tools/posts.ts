...
import { CannyPost } from '../client/types.js';
...
export const getPostsTool = {
  ...
  handler: async (args: unknown, client: CannyClient) => {
    const { boardId, limit, skip, status, search, sort, tagIds, categoryIds } = validateToolInput<GetPostsInput>(args, GetPostsSchema);
...
function normalizePost(post: CannyPost): NormalizedPost {
  const createdAt = post.createdAt || (post.created as string | undefined);
  const updatedAt = post.updatedAt || (post.updated as string | undefined);
...
export const searchPostsTool = {
  ...
  handler: async (args: unknown, client: CannyClient) => {
    const { query, boardIds, limit, status, skip } = validateToolInput<SearchPostsInput>(args, SearchPostsSchema);
